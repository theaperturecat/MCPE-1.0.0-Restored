...
When the Minecraft Dungeons source code leaked, contained in it was a lot of the pocket edition 1.0.0 source code half merged into the unreal game engine. The earliest commit in that repo contains the most, but a lot is still missing or commented out(Just to get it to compile?). I have uncommented a lot and fixed a lot, but there is still a lot more code missing+broken, so obviously you can't compile mcpe 1.0.0 with it yet. It still could be used to make sdk's, backport features to LCE/0.6.2 and it might even be possible to compile mcpe 1.0.0 from it eventually.
