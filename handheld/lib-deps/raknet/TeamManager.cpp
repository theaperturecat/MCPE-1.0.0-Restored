...
void TM_World::DereferenceTeam(TM_Team *team, NoTeamId noTeamSubcategory)
{
	...
	for (i=0; i < teams.Size(); i++)
	{
		if (teams[i]==team)
		{
			TM_Team *foundTeam = teams[i];
			while (foundTeam->teamMembers.Size())
			{
				foundTeam->teamMembers[foundTeam->teamMembers.Size()-1]->LeaveTeam(foundTeam, noTeamSubcategory);
			}
			teams.RemoveAtIndex(i);

			teamsHash.Remove(foundTeam->GetNetworkID(),_FILE_AND_LINE_);
