...
void GetMyIP_Windows_Linux_IPV4And6( SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS] )
{
	...
	int ret = getaddrinfo(ac, "", &hints, &servinfo);
	...
	if (ret == 0) {
		for (idx = 0, aip = servinfo; aip != NULL && idx < MAXIMUM_NUMBER_OF_INTERNAL_IDS; aip = aip->ai_next, idx++) {
			if (aip->ai_family == AF_INET) {
				...
				memcpy(&addresses[idx].address.addr4, ipv4, sizeof(addresses[idx].address.addr4));
			}
			else if(aip->ai_family == AF_INET6) {
				struct sockaddr_in6 *ipv6 = (struct sockaddr_in6 *)aip->ai_addr;
				memcpy(&addresses[idx].address.addr6, ipv6, sizeof(addresses[idx].address.addr6));
			}
...
void GetMyIP_Windows_Linux_IPV4( SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS] )
{
	...
	addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS] = UNASSIGNED_SYSTEM_ADDRESS;
...
void GetMyAdapters_Windows_Linux(NetworkAdapter adapters[MAXIMUM_NUMBER_OF_ADAPTERS]) {
	SocketLayer::GetMyAdapters(adapters);
}
