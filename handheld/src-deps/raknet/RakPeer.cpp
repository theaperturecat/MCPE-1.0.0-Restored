...
// Maximum number of characters in a string representation of any address
// 8 groups of 4 digits separated by 7 ':' then '|' to delimit the port, which is up to 5 digits
static const int MAX_STRING_ADDRESS_SIZE = 4*8 + 7 + 1 + 5;
...
RakPeer::RakPeer()
{
	...
	// Array is actually size (MAXIMUM_NUMBER_OF_INTERNAL_IDS+1) to always null terminate
...
void RakPeer::SendNatTraversalMessage(const SystemAddress target)
{
	if (IsActive() == false)
		return;

	RakNet::BitStream bsOut(sizeof(unsigned char) + sizeof(OFFLINE_MESSAGE_DATA_ID));
	bsOut.Write((MessageID)ID_NAT_TRAVERSAL_PING);
	bsOut.WriteAlignedBytes((const unsigned char*)OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID));
	unsigned int realIndex = GetRakNetSocketFromUserConnectionSocketIndex(0);

	RNS2_SendParameters bsp;
	bsp.data = (char*)bsOut.GetData();
	bsp.length = bsOut.GetNumberOfBytesUsed();
	bsp.systemAddress = target;
	if (bsp.systemAddress == UNASSIGNED_SYSTEM_ADDRESS)
		return;
	bsp.systemAddress.FixForIPVersion(socketList[realIndex]->GetBoundAddress());
	unsigned i;
	for (i = 0; i < pluginListNTS.Size(); i++)
		pluginListNTS[i]->OnDirectSocketSend((const char*)bsOut.GetData(), bsOut.GetNumberOfBitsUsed(), bsp.systemAddress);
...
//		return;
	...
	*/
	...
	unsigned i;
#if RAKNET_SUPPORT_IPV6==1
	// If the address is IPv6 and the prefix is the reserved multicast address space, then send the ping on all availble interfaces
	if (bsp.systemAddress.GetIPVersion() == 6 && bsp.systemAddress.IsMulticastAddress()) {
		
		unsigned int adapterCount = GetNumberOfAdapters();
		for (unsigned int j = 0; j < adapterCount; ++j) {
			NetworkAdapter& adapter = GetLocalAdapter(j);
			
			// Send out on all adapters that are capable of multicasting.
			if (!adapter.isDisabled && (adapter.attributeFlags | IS_MULTICAST)) {
				socketList[realIndex]->SetMulticastInterface(adapter.interfaceIndex);

				for (i = 0; i < pluginListNTS.Size(); i++)
					pluginListNTS[i]->OnDirectSocketSend((const char*)bitStream.GetData(), bitStream.GetNumberOfBitsUsed(), bsp.systemAddress);
				RNS2SendResult result = socketList[realIndex]->Send(&bsp, _FILE_AND_LINE_);
                
                // If the multicast send errored, disable the adapter
				if (result < 0) {
					adapter.isDisabled = true;
					RAKNET_DEBUG_PRINTF("Disabling Network Adapter Index %d from multicasting", adapter.interfaceIndex);
				}
            }
		}

		// If there were no adapters, send out on default index and pray
		if (adapterCount == 0) {
			socketList[realIndex]->SetMulticastInterface(0);
...
{
	// generate a new GUID identifier as long as we are not active
	if (!IsActive()) {
		GenerateGUID();
	}
}
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Description:
// Returns the number of adapters we have
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
unsigned int RakPeer::GetNumberOfAdapters(void)
{
	if (IsActive() == false) {
		FillAdapterList();
	}
	...
	while (adapterList[i].interfaceIndex != -1) {
		i++;
	}
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Returns a network adapter at index 0 to GetNumberOfAdapters-1
// \param[in] index index into the list of network adapters
// \return The local network adapter at this index
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
NetworkAdapter& RakPeer::GetLocalAdapter(unsigned int index)
{
	if (IsActive() == false)
	{
...
/*
	...
	else if (((unsigned char)data[0] == ID_PONG_ADDRESS_INFO) &&
		(size_t) length >= sizeof(MessageID) + sizeof(OFFLINE_MESSAGE_DATA_ID))
	{
		*isOfflineMessage = memcmp(data + sizeof(MessageID), OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
	}
	else if ((
		((unsigned char)data[0] == ID_NAT_TRAVERSAL_PING) ||
		((unsigned char)data[0] == ID_NAT_TRAVERSAL_PONG)) &&
		(size_t)length >= sizeof(MessageID) + sizeof(OFFLINE_MESSAGE_DATA_ID)) 
	{
		*isOfflineMessage = memcmp(data + sizeof(MessageID), OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
	}
	else if (((unsigned char)data[0] == ID_REQUEST_WEBSOCKET_CONNECTION) &&
		(size_t)length >= sizeof(MessageID) + MAX_STRING_ADDRESS_SIZE + sizeof(OFFLINE_MESSAGE_DATA_ID))
	{
		*isOfflineMessage = memcmp(data + sizeof(MessageID), OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
	}
	else if (((unsigned char)data[0] == ID_ACK_FAILED_WEBSOCKET_REQUEST) && 
		(size_t)length >= sizeof(MessageID) + sizeof(OFFLINE_MESSAGE_DATA_ID))
	{
		*isOfflineMessage = memcmp(data + sizeof(MessageID), OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
	}
	...
	if (*isOfflineMessage)
	{
			...
			(unsigned char)(data)[0] == (MessageID)ID_INCOMPATIBLE_PROTOCOL_VERSION)
		{
			...
			if (connectionAttemptCancelled)
			{
				...
				unsigned packetSize = sizeof( char );
				if ((unsigned char)(data)[0] == (MessageID)ID_INCOMPATIBLE_PROTOCOL_VERSION)
					packetSize += sizeof( char );

				packet=rakPeer->AllocPacket(packetSize, _FILE_AND_LINE_);
				packet->data[ 0 ] = data[0]; // Attempted a connection and couldn't
				if ((unsigned char)(data)[0] == (MessageID)ID_INCOMPATIBLE_PROTOCOL_VERSION)
					packet->data[ 1 ] = data[1]; // Version of remote system
				packet->bitSize = ( sizeof( char ) * 8);
				packet->systemAddress = systemAddress;
				packet->guid=guid;
				rakPeer->AddPacketToProducer(packet);
		...
		{
			// parse out the relevant info from the network packet
			RakNet::BitStream bsIn((unsigned char*)data, length, false);
			bsIn.IgnoreBytes(sizeof(unsigned char));
			bsIn.IgnoreBytes(sizeof(OFFLINE_MESSAGE_DATA_ID));
			RakNet::SystemAddress ourExternalAddress;
			bsIn.Read(ourExternalAddress);

			// create the packet we'll provide to the game layer
			int newPacketLength = length - sizeof(OFFLINE_MESSAGE_DATA_ID);
			packet = rakPeer->AllocPacket(newPacketLength, _FILE_AND_LINE_);

			// write important data into the game's packet
			RakNet::BitStream bsOut(packet->data, packet->length, false);
			bsOut.ResetWritePointer();
			bsOut.Write((unsigned char)ID_PONG_ADDRESS_INFO);
			bsOut.Write(ourExternalAddress);
			packet->systemAddress = systemAddress;
			packet->systemAddress.systemIndex = (SystemIndex)rakPeer->GetIndexFromSystemAddress(systemAddress, true);
			packet->guid.systemIndex = packet->systemAddress.systemIndex;
			
			// send the packet to the game
			rakPeer->AddPacketToProducer(packet);
		}
		else if ((unsigned char)(data)[0] == ID_NAT_TRAVERSAL_PING) 
		{
			// if we get this packet, let the game layer know it came in
			packet = rakPeer->AllocPacket(sizeof(MessageID), _FILE_AND_LINE_);
			RakNet::BitStream bsOut(packet->data, packet->length, false);
			bsOut.ResetWritePointer();
			bsOut.Write((unsigned char)ID_NAT_TRAVERSAL_PING);
			packet->systemAddress = systemAddress;
			packet->systemAddress.systemIndex = (SystemIndex)rakPeer->GetIndexFromSystemAddress(systemAddress, true);
			packet->guid.systemIndex = packet->systemAddress.systemIndex;
			...
			// and reply with a pong to the sending system
			RakNet::BitStream bsPong;
			bsPong.Write((unsigned char)ID_NAT_TRAVERSAL_PONG);
			bsPong.WriteAlignedBytes((unsigned char*)OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID));

			RNS2_SendParameters bsp;
			bsp.data = (char*)bsPong.GetData();
			bsp.length = bsPong.GetNumberOfBytesUsed();
			bsp.systemAddress = systemAddress;
			rakNetSocket->Send(&bsp, _FILE_AND_LINE_);
		}
		else if ((unsigned char)(data)[0] == ID_NAT_TRAVERSAL_PONG) {
			// if we get this packet, let the game layer know it came in
			packet = rakPeer->AllocPacket(sizeof(MessageID), _FILE_AND_LINE_);
			RakNet::BitStream bsOut(packet->data, packet->length, false);
			bsOut.ResetWritePointer();
			bsOut.Write((unsigned char)ID_NAT_TRAVERSAL_PONG);
			packet->systemAddress = systemAddress;
			packet->systemAddress.systemIndex = (SystemIndex)rakPeer->GetIndexFromSystemAddress(systemAddress, true);
			packet->guid.systemIndex = packet->systemAddress.systemIndex;
			...
			// Extract data from incoming packet.
			RakNet::BitStream bsIn((unsigned char*)data, length, false);
			bsIn.IgnoreBytes(sizeof(MessageID));
			bsIn.IgnoreBytes(sizeof(OFFLINE_MESSAGE_DATA_ID));

			RakNet::SystemAddress websocketServer;
			char addressBuffer[MAX_STRING_ADDRESS_SIZE];
			bsIn.Read(static_cast<char*>(addressBuffer), MAX_STRING_ADDRESS_SIZE);

			if(websocketServer.FromString(addressBuffer)) {
				// Read credentials
				size_t unread = bsIn.GetNumberOfUnreadBits()/8;
				std::string buffer;
				buffer.resize(unread);
				bsIn.Read(&buffer[0], unread);

				// Let the game layer know about this packet.
				int gamePacketLength = length - sizeof(OFFLINE_MESSAGE_DATA_ID);
				packet = rakPeer->AllocPacket(gamePacketLength, _FILE_AND_LINE_);

				RakNet::BitStream bsOut(packet->data, packet->length, false);
				bsOut.ResetWritePointer();
				bsOut.Write((unsigned char)ID_REQUEST_WEBSOCKET_CONNECTION);
				bsOut.Write(addressBuffer, MAX_STRING_ADDRESS_SIZE);
				bsOut.Write(buffer.data(), buffer.size());
				packet->systemAddress = systemAddress;
				packet->systemAddress.systemIndex = (SystemIndex)rakPeer->GetIndexFromSystemAddress(systemAddress, true);
				packet->guid.systemIndex = packet->systemAddress.systemIndex;
			...
			RakNet::BitStream bsOut(packet->data, packet->length, false);
			bsOut.ResetWritePointer();
			bsOut.Write((unsigned char)ID_ACK_FAILED_WEBSOCKET_REQUEST);
			packet->systemAddress = systemAddress;
			packet->systemAddress.systemIndex = (SystemIndex)rakPeer->GetIndexFromSystemAddress(systemAddress, true);
			packet->guid.systemIndex = packet->systemAddress.systemIndex;
...
void RakPeer::FillAdapterList(void) {
	if (adapterList[0].interfaceIndex != -1) {
		return;
	}

	// Fill out adapterList structure
#if  !defined(WINDOWS_STORE_RT)
	RakNetSocket2::GetMyAdapters(adapterList);
#endif
}
