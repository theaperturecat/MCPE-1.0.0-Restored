	...
	int searchalltypes)
{
	...
	int linklocal = 1;
	char ttl = 2; /* UDA v1.1 says :
				  The TTL for the IP packet SHOULD default to 2 and
				  SHOULD be configurable. */

	if (error)
		*error = UPNPDISCOVER_UNKNOWN_ERROR;
#if !defined(_WIN32) && !defined(__amigaos__) && !defined(__amigaos4__) && !defined(ANDROID) && !defined(__APPLE__)
...
#ifdef _WIN32
	sudp = (int)socket(ipv6 ? PF_INET6 : PF_INET, SOCK_DGRAM, IPPROTO_UDP);
#else
	...
	MIB_IPFORWARD_ROW2 ip_forward2;
	SOCKADDR_INET destAddress;

	memset(&destAddress, 0, sizeof(destAddress));
	destAddress.Ipv4.sin_addr.s_addr = inet_addr("104.66.203.12"); // www.xbox.com is 104.66.203.12
	destAddress.si_family = AF_INET;
	int erro = GetBestRoute2(
		NULL,
		0,
		NULL,
		&destAddress,
	...
	if (erro == NO_ERROR) {
		...
		int Result = GetUnicastIpAddressTable(AF_INET, &pipTable);
		if (Result == NO_ERROR) {
			for (i = 0; i < pipTable->NumEntries; i++) {
				if (pipTable->Table[i].InterfaceIndex == ip_forward2.InterfaceIndex)
				{
					struct in_addr mc_if;
					memset(&mc_if, 0, sizeof(mc_if));
					mc_if.s_addr = pipTable->Table[i].Address.Ipv4.sin_addr.S_un.S_addr;
					if (setsockopt(sudp, IPPROTO_IP, IP_MULTICAST_IF, (const char *)&mc_if, sizeof(mc_if)) < 0) {
						PRINT_SOCKET_ERROR("setsockopt");
					}
					((struct sockaddr_in *)&sockudp_r)->sin_addr.s_addr = pipTable->Table[i].Address.Ipv4.sin_addr.S_un.S_addr;
				}
			}
			if (pipTable != NULL) {
