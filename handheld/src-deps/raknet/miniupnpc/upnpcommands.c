...
{
	return s ? ((int)STRTOUI(s, NULL, 0)) : 0;
}
			...
			char * reservedPort)
{
	...
	// Successful port mapping
		...
		// if we passed in "0" as the external port (wildcard == give me any port), 
		//  get the port# that was reserved for us - or fail if we cant get it
		if (strncmp(extPort, "0", strlen(extPort)) == 0) {
			char *p;
			p = GetValueFromNameValueList(&pdata, "NewReservedPort");
			if (p) {
				strncpy(reservedPort, p, MAXIMUM_PORTNAME_LENGTH);
				reservedPort[5] = '\0';
				ret = UPNPCOMMAND_SUCCESS;
			}
			else {
		...
		// we requested a specific port - return that
		else {
			// TODO: [10/2/2016 mfrost]  I _think_ that we might be able to always check the "NewReservedPort" return value, but cannot confirm that
			//		  for now, don't change the current behavior for this case
			strncpy(reservedPort, extPort, MAXIMUM_PORTNAME_LENGTH);
			reservedPort[5] = '\0';
			ret = UPNPCOMMAND_SUCCESS;
		}
	}
