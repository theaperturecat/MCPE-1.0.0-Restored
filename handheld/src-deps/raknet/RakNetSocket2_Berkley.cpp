...
void RNS2_Berkley::SetBroadcastSocket(int broadcast)
{
	...
	// Enable/disable multicast if the socket is ipv6
	SystemAddress socketAddress;
	this->GetSystemAddressIPV4And6(rns2Socket, &socketAddress);
	if (socketAddress.GetIPVersion() == 6) {
		// IPv6 multicast address & membership info for link-local all routers group
		SystemAddress llnMulticast;
		llnMulticast.FromString("FF02::1", '|', 6);
		
		// Get all available interface adapters.
		NetworkAdapter adapterList[MAXIMUM_NUMBER_OF_ADAPTERS + 1];
		SocketLayer::GetMyAdapters(adapterList);
		
		int adapterCount = 0;
		while (adapterList[adapterCount].interfaceIndex != -1) {
			adapterCount++;
		}

		// If there were no available adapters, then use the default adapter instead.
		if (adapterCount == 0) {
			adapterList[0].interfaceIndex = 0;
			adapterList[0].attributeFlags &= IS_MULTICAST;
			adapterCount = 1;
		}

		// Listen (or stop listening) for multicast packets on all available interfaces
		for (int i = 0; i < adapterCount; ++i) {
			const NetworkAdapter& adapter = adapterList[i];

			if (adapter.attributeFlags | IS_MULTICAST) {
				ipv6_mreq membership;
				membership.ipv6mr_multiaddr = llnMulticast.address.addr6.sin6_addr;
				membership.ipv6mr_interface = adapter.interfaceIndex;

				if (broadcast) {
					// Join the multicast group on the specified interface
					setsockopt__(rns2Socket, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char*)&membership, sizeof(membership));
					RAKNET_DEBUG_PRINTF("[IPv6] multicast listen on interface %d\n", adapter.interfaceIndex);
				}
				else {
					// disable the receiving of multicast packets on the default interface
					setsockopt__(rns2Socket, IPPROTO_IPV6, IPV6_LEAVE_GROUP, (char*)&membership, sizeof(membership));
				}
			}
		}

		int multicastHops = 1;
		char multicastLoopback = 0;
		if (broadcast) {

			// Set the number of hops to 1 - probably unnecessary since link-local won't be sent beyond the local network.
			setsockopt__(rns2Socket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (char*)&multicastHops, sizeof(multicastHops));

			// Turn off Multicast loopback so that a host doesn't receive it's own multicast packets
			setsockopt__(rns2Socket, IPPROTO_IPV6, IP_MULTICAST_LOOP, &multicastLoopback, sizeof(multicastLoopback));
		}
	}
#endif
}

void RNS2_Berkley::SetMulticastInterface(int interfaceIndex) {
#if RAKNET_SUPPORT_IPV6==1
	setsockopt__(rns2Socket, IPPROTO_IPV6, IPV6_MULTICAST_IF, (char*)&interfaceIndex, sizeof(interfaceIndex));
#endif
}
...
RNS2BindResult RNS2_Berkley::BindSharedIPV4And6( RNS2_BerkleyBindParameters *bindParameters, const char *file, unsigned int line ) {
	...
	for (aip = servinfo; aip != NULL; aip = aip->ai_next)
	{
		...
		// getaddrinfo() gave us.
		rns2Socket = (RNS2Socket)socket__(aip->ai_family, aip->ai_socktype, aip->ai_protocol);
		...
		{
			// Is this valid?
			memcpy(&boundAddress.address.addr6, aip->ai_addr, sizeof(*aip->ai_addr));
		...
		{
			closesocket__(rns2Socket);
			rns2Socket = -1; // Invalidate the socket so it doesn't get closed twice.
		}
	}
...
void RNS2_Berkley::RecvFromBlockingIPV4(RNS2RecvStruct *recvFromStruct)
{
	...
	if (recvFromStruct->bytesRead<=0)
	{
		...
		{
#if 0//defined(_WIN32) && !defined(_XBOX) && !defined(_XBOX_720_COMPILE_AS_WINDOWS) && !defined(X360) && defined(_DEBUG) && !defined(_XBOX_720_COMPILE_AS_WINDOWS) && !defined(WINDOWS_PHONE_8)
			LPVOID messageBuffer;
			FormatMessage( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
				NULL, dwIOError, MAKELANGID( LANG_NEUTRAL, SUBLANG_DEFAULT ),  // Default language
				( LPTSTR ) & messageBuffer, 0, NULL );
