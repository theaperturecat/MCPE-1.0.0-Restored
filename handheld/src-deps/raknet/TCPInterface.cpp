...
SystemAddress TCPInterface::Connect(const char* host, unsigned short remotePort, bool block, unsigned short socketFamily, const char *bindAddress)
{
	...
	else
	{
		...
		// Assume the string conversion will fail, if it succeeds errorcode will be overriden with the success of the thread creation.
		int errorCode = -1;
		
		// Help the FromString function by specifying what IP version is expected, otherwise resolving hostnames can result in the wrong ip address.
		int ipVersion = socketFamily == AF_INET ? 4 : (socketFamily == AF_INET6 ? 6 : 0);

		// The FromString function will try to resolve host names as well as convert the string to a byte representation of the address. 
		// If either case fails then false is returned (the address isn't valid). The connection should also fail at that point.
		if (s->systemAddress.FromStringExplicitPort(host, remotePort, ipVersion)) {
...
__TCPSOCKET__ TCPInterface::SocketConnect(const char* host, unsigned short remotePort, unsigned short socketFamily, const char *bindAddress)
{
	...
	// Increase the receiving buffer size to 256Kb. Could be less if limited by platform.
	int sock_opt = 1024 * 256;
	setsockopt__(sockfd, SOL_SOCKET, SO_RCVBUF, (char *)& sock_opt, sizeof(sock_opt));
