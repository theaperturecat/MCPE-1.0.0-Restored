...
void RakNetSocket2::GetMyAdapters(NetworkAdapter adapters[MAXIMUM_NUMBER_OF_ADAPTERS])
{
#if defined(WINDOWS_STORE_RT)
	RNS2_WindowsStore8::GetMyAdapters(adapters);
#elif defined(__native_client__)
	RNS2_NativeClient::GetMyAdapters(adapters);
#elif defined(_WIN32)
	RNS2_Windows::GetMyAdapters(adapters);
#else
	RNS2_Linux::GetMyAdapters(adapters);
#endif
}
...
void RakNetSocket2::SetMulticastInterface(int interfaceIndex){}
...
void RNS2_NativeClient::GetMyAdapters(NetworkAdapter adapters[MAXIMUM_NUMBER_OF_ADAPTERS]) { adapters[0].interfaceIndex = -1; RakAssert("GetMyAdapters Unsupported?" && 0); }
...
#define INVALID_RNS2SOCKET  (RNS2Socket)(~0)

RNS2_Berkley::RNS2_Berkley()
{
	rns2Socket= INVALID_RNS2SOCKET;
}
RNS2_Berkley::~RNS2_Berkley()
{
	if (rns2Socket!= INVALID_RNS2SOCKET)
	{
		/*
#if defined(__APPLE__)
...
void RNS2_Windows::GetMyAdapters(NetworkAdapter adapters[MAXIMUM_NUMBER_OF_ADAPTERS]) { return GetMyAdapters_Windows_Linux(adapters); }
...
void RNS2_Linux::GetMyAdapters(NetworkAdapter adapters[MAXIMUM_NUMBER_OF_ADAPTERS]) { return GetMyAdapters_Windows_Linux(adapters); }
#endif // Linux
