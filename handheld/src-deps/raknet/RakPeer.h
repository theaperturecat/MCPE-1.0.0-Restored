...
public:
	...
	/// \brief Send a simple packet to the remote system, useful for Nat traversal
	/// \param[in] target Which system to contact
	void SendNatTraversalMessage(const SystemAddress target);
	...
	/// Re-generate a new GUID RaknetID
	void resetMyGUID();
	...
	/// \brief Returns the number of network adapters this system has.
	/// \details Get the actual adapter info from GetLocalAdapter()
	unsigned GetNumberOfAdapters(void);

	/// Returns a network adapter at index 0 to GetNumberOfAdapters-1 in adapterList array.
	/// \param[in] index index into the list of adapters
	/// \return The local adapter at this index
	NetworkAdapter& GetLocalAdapter(unsigned int index);
...
//		char remote_challenge[cat::EasyHandshake::CHALLENGE_BYTES];
	...
	// size is (MAXIMUM_NUMBER_OF_INTERNAL_IDS + 1) to always null terminate the array
	SystemAddress ipList[ MAXIMUM_NUMBER_OF_INTERNAL_IDS +1 ];
