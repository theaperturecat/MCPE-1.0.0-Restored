...
struct RAK_DLL_EXPORT SystemAddress
{
	...
	/// Sets the port in host order (this is what you normally use)
	...
	bool IsValidIPAddress(void);
	bool IsLinkLocalAddress(void);
...
enum AdapterAttributes {
	NO_ATTRIBUTES = 0x00,
	IS_MULTICAST = 0x01,
	IS_LOOPBACK = 0x02,
};

struct RAK_DLL_EXPORT NetworkAdapter {
	unsigned int attributeFlags;
	int interfaceIndex;
    bool isDisabled;
	SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS + 1];
