...
RakNet::RakString SocketLayer::GetSubNetForSocketAndIp( RakNet::RakString inIpString)
{
	...
	__UDPSOCKET__ s = socket__(AF_INET, SOCK_DGRAM, 0);
...
void SocketLayer::GetMyAdapters(NetworkAdapter adapters[MAXIMUM_NUMBER_OF_ADAPTERS])
{
#if defined(ANDROID)
	// ANDROID doesn't support getifaddrs, should use netlink instead?
#elif defined(_WIN32)
	// Any other Windows platform should end up here (e.g. Win10 UWP)
	...
	// We aren't willing to commit to a larger buffer size to store all the data we need for adapters, so we must fail if the max is exceeded. 
	const ULONG maxBufferLength = 30000;	// 30 KB - has to be enough, right?
	// Allocate a buffer that's big enough to store 10 adapters, each with 20 addresses, assume ipv6 as worst case which is 28 bytes.
	// Of course this is oversimplifying a bit considering size of pointers and other struct members aren't being factored in.
	ULONG outBufLen = 10 * 20 * 28; // 5600
	ULONG Iterations = 0;
	ULONG maxTries = 3;
	...
	do {
		...
		// If the buffer length isn't enough to hold the addresses, it'll return with ERROR_BUFFER_OVERFLOW and set the outBufLen to the size required.
		// No data is set to pAddresses as a result of a ERROR_BUFFER_OVERFLOW.
	...
	} while ((dwRetVal == ERROR_BUFFER_OVERFLOW) && (Iterations < maxTries) && (outBufLen <= maxBufferLength));

	// Error occurred retrieving interfaces
	if (dwRetVal != NO_ERROR) {
		return;
	}

	// Use a copy of the pointer to the start of addresses
	pCurrAddresses = pAddresses;

	// Iterate through all interfaces
	while (pCurrAddresses && count < MAXIMUM_NUMBER_OF_ADAPTERS) {
		// Only process adapters that are UP
		if (pCurrAddresses->OperStatus == IfOperStatusUp) {

			// Get Interface Attributes
			unsigned int attributes = 0;
			attributes = (!(pCurrAddresses->Flags & IP_ADAPTER_NO_MULTICAST)) ? attributes | (unsigned int)IS_MULTICAST : attributes;

			// this would be how we check for Loopback but it's not available in UWP, pCurrAddresses->IfType == IF_TYPE_SOFTWARE_LOOPBACK
			// Instead, we test the address of the interface and if loopback, then the interface is a software loopback interface. (see below)

			// Get Unicast addresses
			pUnicast = pCurrAddresses->FirstUnicastAddress;
			if (pUnicast != NULL) {
				for (int u = 0; pUnicast != NULL && u < MAXIMUM_NUMBER_OF_INTERNAL_IDS;u++) {
					if (pUnicast->Address.lpSockaddr->sa_family == AF_INET) {
						memcpy(&adapters[count].addresses[u].address.addr4, (struct sockaddr_in *)pUnicast->Address.lpSockaddr, sizeof(struct sockaddr_in));
					}
#if RAKNET_SUPPORT_IPV6==1
					else if (pUnicast->Address.lpSockaddr->sa_family == AF_INET6) {
						memcpy(&adapters[count].addresses[u].address.addr4, (struct sockaddr_in6 *)pUnicast->Address.lpSockaddr, sizeof(struct sockaddr_in6));
					}
#endif
					else {
						// Address wasn't added
						u--;
					}
				...
				// If addresses were obtained for this adapter interface and they are loopback addresses, then this is a software loopback interface
				attributes = (adapters[count].addresses[0] != UNASSIGNED_SYSTEM_ADDRESS && adapters[count].addresses[0].IsLoopback()) ? attributes | (unsigned int)IS_LOOPBACK : attributes;
			}

			adapters[count].attributeFlags = attributes;

			// Get ipv6 index for this interface (used in multicasting)
			adapters[count++].interfaceIndex = pCurrAddresses->Ipv6IfIndex;
		}
...
#else
	// Unix/Linux based platforms will run this portion of the code.
	// e.g. MacOS/iOS. Android doesn't implement getifaddrs natively so it's implementation is in the Java layer.

	int count = 0;

	struct ifaddrs *ifRoot = NULL, *ifa = NULL;
	int error = 0;

	error = getifaddrs(&ifRoot);
	if (error == 0) {
		for (ifa = ifRoot; ifa != NULL && count < MAXIMUM_NUMBER_OF_ADAPTERS; ifa = ifa->ifa_next) {
			
			// Skip interfaces that aren't active or that don't have any address data
			if ((ifa->ifa_flags & IFF_UP) && ifa->ifa_addr != NULL) {
				// The addresses are returned as a link list and nodes may point to the same interface.
				// Instead of using a Map and taking on the DS_Map implementation in RakNet, just do a linear search.
				// The cost should be neglible given the small array size.
				int ifIndex = if_nametoindex(ifa->ifa_name);
				int adapterIndex = -1;

				for (int i = 0; i < count; ++i) {
					// Find a match
					if (adapters[i].interfaceIndex == ifIndex) {
						adapterIndex = i;
						break;
					}
				}

				// No match found, this is a new interface.
				if (adapterIndex == -1) {
                    RAKNET_DEBUG_PRINTF("Network Adapter Found! Index: %d Name: %s", ifIndex, ifa->ifa_name);
                    
                    adapterIndex = count;
					adapters[count++].interfaceIndex = ifIndex;

					// Get Interface Attributes
					unsigned int attributes = 0;
					attributes = ifa->ifa_flags & IFF_MULTICAST ? attributes | (unsigned int)IS_MULTICAST : attributes;
					attributes = ifa->ifa_flags & IFF_LOOPBACK ? attributes | (unsigned int)IS_LOOPBACK : attributes;
					adapters[adapterIndex].attributeFlags = attributes;
				}

				// Get next address index
				int addressIndex = 0;
				for (; adapters[adapterIndex].addresses[addressIndex] != UNASSIGNED_SYSTEM_ADDRESS; ++addressIndex);

				if (addressIndex >= MAXIMUM_NUMBER_OF_INTERNAL_IDS) {
					continue;
				}

				// Get address data
				if (ifa->ifa_addr->sa_family == AF_INET) {
					memcpy(&adapters[adapterIndex].addresses[addressIndex].address.addr4, (struct sockaddr_in *)ifa->ifa_addr, sizeof(struct sockaddr_in));
				}
#if RAKNET_SUPPORT_IPV6==1
				else if (ifa->ifa_addr->sa_family == AF_INET6) {
					memcpy(&adapters[adapterIndex].addresses[addressIndex].address.addr4, (struct sockaddr_in6 *)ifa->ifa_addr, sizeof(struct sockaddr_in6));
				}
#endif
			}
		}
		...
		error = errno;
		RAKNET_DEBUG_PRINTF("getifaddrs failed: Error code - %d\n", error);
	}
...
void GetMyIP_Win32( SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS] )
{
	...
	{
 #if 0//defined(_WIN32) && !defined(WINDOWS_PHONE_8)
...
void SocketLayer::GetSystemAddress_Old ( __UDPSOCKET__ s, SystemAddress *systemAddressOut )
{
	...
	*systemAddressOut = UNASSIGNED_SYSTEM_ADDRESS;
	...
	{
#if 0//defined(_WIN32) && defined(_DEBUG) && !defined(WINDOWS_PHONE_8)
		DWORD dwIOError = GetLastError();
		LPVOID messageBuffer;
		FormatMessage( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
			NULL, dwIOError, MAKELANGID( LANG_NEUTRAL, SUBLANG_DEFAULT ),  // Default language
