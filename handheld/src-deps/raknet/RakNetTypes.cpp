...
bool SystemAddress::IsValidIPAddress(void) {
	if (GetIPVersion() == 6) {
		return true;
	}

#if defined(__WIN32__)
	return address.addr4.sin_addr.S_un.S_un_b.s_b1 != 0;
#elif defined(ANDROID)
	return (address.addr4.sin_addr.s_addr & 0xFF) != 0;
#else
	return (address.addr4.sin_addr.s_addr >> 24) != 0;
#endif
}
...
bool SystemAddress::IsLinkLocalAddress(void) {
	// 169.254.0.0/16
	if (GetIPVersion() == 4) {
		const static unsigned char linkLocalIPv4[2] = { (unsigned char)169, (unsigned char)254 };
		if (memcmp(&address.addr4.sin_addr, linkLocalIPv4, sizeof(linkLocalIPv4)) == 0) {
			return true;
		}
	}
#if RAKNET_SUPPORT_IPV6==1
	// fe80::/64
		...
		const static unsigned char linkLocalIPv6[8] = {(unsigned char)0xfe, (unsigned char)0x80,0,0,0,0,0,0 };
		if (memcmp(&address.addr6.sin6_addr, linkLocalIPv6, sizeof(linkLocalIPv6)) == 0) {
			return true;
		}
	}
#endif
...
bool SystemAddress::IsMulticastAddress(void) {
	// 224.0.0.0 - 239.255.255.255
	if (GetIPVersion() == 4) {
		const static unsigned char multicastIPv4Min[1] = { (unsigned char)224 };
		const static unsigned char multicastIPv4Max[1] = { (unsigned char)239 };
		if (memcmp(&address.addr4.sin_addr, multicastIPv4Min, sizeof(multicastIPv4Min)) >= 0 &&
			memcmp(&address.addr4.sin_addr, multicastIPv4Max, sizeof(multicastIPv4Max)) <= 0) {
			return true;
		}
	}
#if RAKNET_SUPPORT_IPV6==1
	// high-order octet starts with 0xFF
	else {
		const static unsigned char multicastIPv6[1] = { (unsigned char)0xff };
		if (memcmp(&address.addr6.sin6_addr, multicastIPv6, sizeof(multicastIPv6)) == 0) {
			return true;
		}
	}
#endif
...
bool SystemAddress::FromString(const char *str, char portDelineator, int ipVersion)
{
	...
	char ipPart[MAXIMUM_HOSTNAME_LENGTH];
	char portPart[32];
	int i=0,j;

	if (strlen(str) >= sizeof(ipPart)) {
		RAKNET_DEBUG_PRINTF("Hostname: %s\n", str);
		RakAssert("Hostname is longer than the maximum hostname length." && 0);
	}

	// TODO - what about 255.255.255.255?
	if (ipVersion==4 && strcmp(str, IPV4_LOOPBACK)==0)
	{
		strcpy(ipPart,IPV4_LOOPBACK);
	}
	else if (ipVersion==6 && strcmp(str, IPV6_LOOPBACK)==0)
	{
	...
	// This call fails on Android if an empty string is used, null seems to work cross platform
...
NetworkAdapter::NetworkAdapter() {
	interfaceIndex = -1;
    isDisabled = false;
	attributeFlags = (unsigned int)NO_ATTRIBUTES;
}

unsigned int NetworkAdapter::GetNumberOfAddresses(void) {
	int i = 0;

	while (addresses[i] != UNASSIGNED_SYSTEM_ADDRESS) {
		i++;
	}
