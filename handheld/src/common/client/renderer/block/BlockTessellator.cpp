...
Color getBlockColour(BlockID id)
{
	switch (id)
	{
	case 0: return Color::from255Range(255, 255, 255);
	case 1: return Color::from255Range(128,128,128 );
	case 3: return Color::from255Range(130,90,50 );
	case 2: return Color::from255Range(90,180,60 );
	case 7: return Color::from255Range(40,40,40 );
	case 8:
	case 9: return Color::from255Range(40,80,200 );
	case 10:
	case 11: return Color::from255Range(255,60,0 );
	case 87: return Color::from255Range(180, 10, 0 );
	default:
...
bool BlockTessellator::tessellateBlockInWorld(const Block& block, const BlockPos& pos, int data) {
	Color base = Color::fromARGB(block.getColor(*mRegion, pos, data));//getBlockColour(block.getId());// * Color::fromARGB(block.getColor(*mRegion, pos, data));
...
bool BlockTessellator::tessellateDoublePlantInWorld(const DoublePlantBlock& block, const BlockPos& p, DataID data, bool preferPoly, bool renderBothSides) {
	...
	if(isTop && type == enum_cast(DoublePlantType::Sunflower)) {
		float angle = Math::cos(seed * .8f) * Math::PI * 0.1f;
...
bool BlockTessellator::tessellateBrewingStandInWorld(const BrewingStandBlock& block, const BlockPos& pos, int data) {
	...
	for(int arm = 0; arm < 3; arm++) {
		float angle = arm * Math::PI * 2.0f / 3.0f + Math::PI * .5f;
...
bool BlockTessellator::tessellateLeverInWorld(const Block& block, const BlockPos& pos, int data) {
	...
	for (int i = 0; i < 8; i++) {
		if (flipped) {
			corners[i].z -= 1 / 16.0f;
			corners[i].xRot(40 * Math::PI / 180);
		} else {
			corners[i].z += 1 / 16.0f;
			corners[i].xRot(-40 * Math::PI / 180);
		}
		if (dir == 0 || dir == 7) {
			corners[i].zRot(180 * Math::PI / 180);
		}
		if (dir == 6 || dir == 0) {
			corners[i].yRot(90 * Math::PI / 180);
		}

		if (dir > 0 && dir < 5) {
			corners[i].y -= 6 / 16.0f;
			corners[i].xRot(90 * Math::PI / 180);

			if (dir == 4) {
				corners[i].yRot(0 * Math::PI / 180);
			}
			if (dir == 3) {
				corners[i].yRot(180 * Math::PI / 180);
			}
			if (dir == 2) {
				corners[i].yRot(90 * Math::PI / 180);
			}
...
bool BlockTessellator::tessellateTripwireHookInWorld(const Block& b, const BlockPos& p, int data) {
	...
	for (int i = 0; i < 8; i++) {
		...
		if (powered) {
			corners[i].xRot(30 * Math::PI / 180);
			corners[i].y -= 7 / 16.0f;
		}
		else if (attached) {
			corners[i].xRot(5 * Math::PI / 180);
			corners[i].y -= 7 / 16.0f;
		}
		else {
			corners[i].xRot(-40 * Math::PI / 180);
	...
	for (int i = 0; i < 8; i++) {
		...
		if (powered) {
			...
			corners[i].xRot(0 * Math::PI / 180);
		}
		else if (attached) {
			corners[i].y += 0.25f / 16.0f;
			corners[i].z -= 2.75f / 16.0f;
			corners[i].xRot(10 * Math::PI / 180);
		}
		else {
			corners[i].xRot(50 * Math::PI / 180);
		}
