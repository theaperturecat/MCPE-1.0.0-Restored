...
void Dimension::init() {
	...
	if (!mLevel.mIsClientSide) {
		...
		const bool enableDisk = true;	//TODO do some file-based option here theaperturecat
...
float Dimension::getSkyDarken(float a) const {
	...
	float brightness = 1 - (cosf(td * Math::PI * 2.f) * 2.f + 0.2f);
...
Brightness Dimension::getOldSkyDarken(float a) {
	float td = getTimeOfDay(a);
	float brightness = 1 - (Math::cos(td * Math::PI * 2) * 2 + 0.5f);
	if (brightness < 0.f) {
		brightness = 0.f;
	}
	if (brightness > 1.f) {
...
Color Dimension::getSunriseColor(float a) const {
	...
	Color sunriseCol;
	float span = 0.4f;
	float tt = Math::cos(td * Math::PI * 2.f) - 0.0f;
	float mid = -0.0f;
	if (tt >= mid - span && tt <= mid + span) {
		float aa = ((tt - mid) / span) * 0.5f + 0.5f;
		float mix = 1 - (((1 - Math::sin(aa * Math::PI))) * 0.99f);
		mix = mix * mix;
...
Color Dimension::getSkyColor(BlockSource& source, const BlockPos& pos, float a) {
	...
	float brightness = Math::clamp(Math::cos(td * Math::PI * 2) * 2 + 0.5f, 0.f, 1.f);
...
Color Dimension::getCloudColor(float a) {
	...
	float brightness = Math::cos(td * Math::PI * 2) * 2.0f + 0.5f;
	if (brightness < 0.0f) {
		brightness = 0.0f;
	}
	if (brightness > 1.0f) {
...
float Dimension::getStarBrightness(float a) {
	...
	float br = 1 - (Math::cos(td * Math::PI * 2) * 2 + 0.75f);
...
float Dimension::getTimeOfDay(int time, float a) const {
	...
	float tdo = td;
	td = 1 - (float)((Math::cos(td * Math::PI) + 1) / 2);
	td = tdo + (td - tdo) / 3.0f;
	return td;
}
