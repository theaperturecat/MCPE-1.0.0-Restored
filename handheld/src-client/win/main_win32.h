...
int main(void) {
	...
	//if (appContext.display == EGL_NO_DISPLAY) { appContext.display = eglGetDisplay((EGLNativeDisplayType)EGL_DEFAULT_DISPLAY); }
	//m_eglDisplay = eglGetDisplay((EGLNativeDisplayType) EGL_DEFAULT_DISPLAY);
	printf("EGLSurface = %p\n", appContext.display);

	bool init = eglInitialize(appContext.display, NULL, NULL);
	printf("EGLInit = %i\n", init);

	bool bound = eglBindAPI(EGL_OPENGL_ES_API);
	printf("EGLBind = %i\n", bound);

	/*EGLint numConfigs;

	// Get number of configs
	eglGetConfigs(appContext.display, NULL, 0, &numConfigs);
	printf("Number of available EGL configs: %d\n", (unsigned int)numConfigs);
	printf("EGL error: %d\n", eglGetError());
	// Allocate array for all configs
	void ** configs = new void *[(unsigned int)numConfigs];
	eglGetConfigs(appContext.display, configs, numConfigs, &numConfigs);
	printf("Number of available EGL configs: %d\n", (unsigned int)numConfigs);
	for (int i = 0; i < 1; i++) {
		EGLint red, green, blue, alpha, depth, surfaceType, renderableType;

		eglGetConfigAttrib(appContext.display, configs[i], EGL_RED_SIZE, &red);
		eglGetConfigAttrib(appContext.display, configs[i], EGL_GREEN_SIZE, &green);
		eglGetConfigAttrib(appContext.display, configs[i], EGL_BLUE_SIZE, &blue);
		eglGetConfigAttrib(appContext.display, configs[i], EGL_ALPHA_SIZE, &alpha);
		eglGetConfigAttrib(appContext.display, configs[i], EGL_DEPTH_SIZE, &depth);
		eglGetConfigAttrib(appContext.display, configs[i], EGL_SURFACE_TYPE, &surfaceType);
		eglGetConfigAttrib(appContext.display, configs[i], EGL_RENDERABLE_TYPE, &renderableType);

		printf("Config %d: R%d G%d B%d A%d Depth%d SurfaceType 0x%x RenderableType 0x%x\n",
			i, red, green, blue, alpha, depth, surfaceType, renderableType);
	}*/

	bool succededconfig = eglChooseConfig(appContext.display, aEGLAttributes, m_eglConfig, 1, &nConfigs);
	printf("EGLConfig = %p\n", m_eglConfig[0]);
	printf("EGLConfig bool = %i\n", succededconfig);
	printf("EGL error: %d\n", eglGetError());
